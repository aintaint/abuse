...
void net_receive();
void net_send(int force);

// from setup ****************************************************************

void showHelp(const char* executableName);
void createRCFile(std::string rcfile);
void readRCFile();
...
std::string abuse_path = "/abuse/";
std::string abuse_save_path = abuse_path;
std::string assetdir = abuse_path;
std::string savedir = assetdir + "savedata/";

#define PACKAGE_NAME "wasmabuse"
#define PACKAGE_VERSION "0.9.2e"
...
void wasmMain() {
    ...
    flags.antialias          = 1;    // Don't anti-alias
    ...
    if( (fd = fopen( savedir.c_str(), "r" )) == NULL ) {
        // FIXME: Add some error checking here
        mkdir( savedir.c_str(), S_IRUSR | S_IWUSR | S_IXUSR );
    }  else  {
        fclose( fd );
    }
//    set_save_filename_prefix( savedir.c_str() );
//    set_filename_prefix( assetdir.c_str() );
    ...
    //********************************************
    std::cout << "Abuse version " << PACKAGE_VERSION << std::endl;
//    show_startup();

//    start_sound(start_argc, start_argv);
    sfx_volume = music_volume = 127;
    sound_avail = sound_init(start_argc, start_argv);

    stat_man = new text_status_manager();

//    set_filename_prefix(abuse_path.c_str());
//    set_save_filename_prefix(abuse_save_path.c_str());

    jrand_init();
    jrand(); // so compiler doesn't complain

    set_spec_main_file("abuse.spe");
//    check_for_lisp(start_argc, start_argv);
    ...
    do {
        ...
        game_net_init(start_argc, start_argv);
        Lisp::Init();

        dev_init(start_argc, start_argv);

        Game *g = new Game(start_argc, start_argv);
        ...
        for (int i = 1; i + 1 < start_argc; i++)
        {
            if (!strcmp(start_argv[i], "-server"))
            {
                if (!become_server(start_argv[i + 1]))
                {
...
//  printFile("/abuse/abuse.lsp");
  ...
  emscripten_set_main_loop(&wasmMain, 0, simulate_infinite_loop);
...
//    char *fastpath = (char *)malloc(strlen(get_save_filename_prefix()) + 13);
    std::string fastpath = "/abuse/fastload.dat";
//    fastpath.insert(0, get_save_filename_prefix(), strlen(get_save_filename_prefix()));
//    sprintf(fastpath, "%sfastload.dat", get_save_filename_prefix());

    unlink(fastpath.c_str());
//    free(fastpath);
...
// ******************************************************************
    ...
    SDL_CreateWindowAndRenderer(640, 400, 0, &window, &renderer);
    surface = SDL_CreateRGBSurface(0, 640, 400, 32, 0, 0, 0, 0);

// ******************************************************************
// setup fs and unpack data
...
void createRCFile(std::string rcfile )
{
    ...
    if( (fd = fopen( rcfile.c_str(), "w" )) != NULL )
    {
...
void readRCFile()
{
    ...
    std::string rcfile = "/abuse/abuserc"; // get_save_filename_prefix();
//    rcfile.insert(0, get_save_filename_prefix(), strlen(get_save_filename_prefix()));

    if( (fd = fopen( rcfile.c_str(), "r" )) != NULL )
    {
